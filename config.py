...
# Defina uma função para mover uma janela para a próxima área de trabalho
keys = [
    ...
    KeyChord(
        [mod], "f", [
            ...
            Key([], "tab", lazy.layout.next(), desc="Move window focus to other window around"),
            Key([], "escape", lazy.ungrab_all_chords(), desc="Close all chords")
        ],
        mode=True
    ),
    KeyChord(
        [mod], "r", [
            ...
            Key([], "j", lazy.layout.shuffle_down(), desc="Move window to the down"),
            Key([], "k", lazy.layout.shuffle_up(), desc="Move window to the up"),
            Key([], "Tab", lazy.window.toggle_floating(), desc='Toggle floating'),
    ...
    Key([], "Print", lazy.spawn(home + "/dotfiles/qtile/scripts/screenshot.sh")),
    # System
    Key([], "F5", lazy.reload_config(), desc="Reload the config"),
    Key([mod1], "F4", lazy.window.kill(), desc="Kill focused window"),
    Key([mod, mod1], "F4", lazy.spawn(home + "/dotfiles/qtile/scripts/powermenu.sh"), desc="Open Powermenu"),

    # Apps
    Key(["control", mod1], "t", lazy.spawn(terminal), desc="Launch terminal"),
    Key([mod1], "F2", lazy.spawn("rofi -show drun"), desc="Launch Rofi"),
    Key([mod], "b", lazy.spawn("sh " + home + "/dotfiles/.settings/browser.sh"), desc="Launch Browser"),
    Key([mod, "shift"], "w", lazy.spawn(home + "/dotfiles/qtile/scripts/wallpaper.sh"),
        desc="Update Theme and Wallpaper"),
    Key([mod, "control"], "w", lazy.spawn(home + "/dotfiles/qtile/scripts/wallpaper.sh select"),
    ...
    Key([], "F11", lazy.window.toggle_fullscreen(), desc="Enable full screen"),
    Key([], "XF86Tools", lazy.spawn("alacritty --class dotfiles-floating -e ~/dotfiles/hypr/settings/settings.sh")),
    Key(["control", mod1], "Right", lazy.screen.next_group(), desc="Switch to next group"),
    Key(["control", mod1], "Left", lazy.screen.prev_group(), desc="Switch to previous group"),
]
...
groups.append(ScratchPad("6", [
    DropDown("chatgpt", "chromium --app=https://chat.openai.com", x=0.3, y=0.1, width=0.40, height=0.4,
